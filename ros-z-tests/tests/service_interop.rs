...
fn test_ros_z_server_ros_z_client() {
    ...
    let client_handle = thread::spawn(|| {
        ...
        let resp = zcli.take_response().expect("Failed to receive response");
...
//     assert!(stdout.contains("sum: 17") || stdout.contains("sum=17"),
//             "Expected sum: 17, got: {}", stdout);
//
...
//             .expect("Failed to create context");
//
//         let node = ctx.create_node("rosz_client")
//             .build()
//             .expect("Failed to create node");
//
//         let zcli = node
...
//         zcli.send_request(&req).expect("Failed to send request");
//
//         let resp = zcli.take_response_timeout(std::time::Duration::from_secs(2)).expect("Failed to receive response");
//         println!("Received response from ROS2: {}", resp.sum);
//
//         resp
//     }).join().expect("Client thread panicked");
//
