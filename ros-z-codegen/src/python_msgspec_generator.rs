...
fn generate_complete_rust_module(
    ...
    for (package_name, package_msgs) in packages {
        code.push_str("#[allow(unsafe_op_in_unsafe_fn, clippy::useless_conversion)]\n");
        code.push_str("mod ");
...
fn generate_pymodule_registration(
    ...
    // Add convenience functions wrapped in a module to suppress macro-generated warnings
    code.push_str(
    ...
    // Add all registered message types
    for (package_name, package_msgs) in packages {
        for msg in package_msgs {
    ...
    // Build match statement for type hashes
    for (package_name, package_msgs) in packages {
        for msg in package_msgs {
            let full_name = format!("{}/msg/{}", package_name, msg.parsed.name);
            let hash = msg.ros2_hash.to_hash_string();
    ...
    code.push_str(
        "            _ => Err(pyo3::exceptions::PyValueError::new_err(format!(\"Unknown message type: {}\", type_name))),\n\
         }\n\
         }\n\
         }\n\
