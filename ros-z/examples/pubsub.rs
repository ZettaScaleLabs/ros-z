...
fn run_publisher(topic: String, duration: Duration, period: Duration, payload_size: usize) -> Result<()> {
    let ctx = ZContextBuilder::default().build()?;
    let node = ctx.create_node("MyNode").build()?;
...
fn main() -> Result<()> {
    ...
    tracing::info!("Begin testing with topic: {}, duration: {duration:?}, payload: {}, rate: {} Hz", args.topic, args.payload, args.rate);
    if args.mode == "sub" {
