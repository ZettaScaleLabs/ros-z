use std::{
    fs::File, path::PathBuf, sync::{atomic::{AtomicBool, Ordering::SeqCst}, Arc}, time::{Duration, Instant}
};

use csv::Writer;
use zenoh::{qos::CongestionControl, Result, Wait};
...
fn main() -> Result<()> {
    ...
    let zsub = session.declare_subscriber("pong").wait()?;
    let zpub = session
