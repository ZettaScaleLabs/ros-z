...
fn extract_type_info_from_class(msg_class: &Bound<'_, PyAny>) -> PyResult<(String, TypeInfo)> {
    ...
    let msg_type: String = msg_class
        ...
        .extract()
        .map_err(|_| pyo3::exceptions::PyTypeError::new_err(
            "Message class __msgtype__ must be a string"
        ))?;
    ...
    let type_hash_str: String = msg_class
        ...
        .extract()
        .map_err(|_| pyo3::exceptions::PyTypeError::new_err(
            "Message class __hash__ must be a string"
        ))?;
...
fn extract_service_type_from_request_class(request_class: &Bound<'_, PyAny>) -> PyResult<(String, TypeInfo)> {
...
fn create_publisher_for_type(
    ...
    match msg_type {
        ...
        "std_msgs/msg/ByteMultiArray" => {
            let pub_builder = node.create_pub_impl::<std_msgs::ByteMultiArray>(topic, Some(type_info))
                .with_qos(qos);
            let zpub = pub_builder.build().map_err(|e| e.into_pyerr())?;
            let wrapper = ZPubWrapper::new(zpub);
            Ok(PyZPublisher::new(Box::new(wrapper), msg_type.to_string()))
...
fn create_subscriber_for_type(
    ...
    match msg_type {
        ...
        "std_msgs/msg/ByteMultiArray" => {
            let sub_builder = node.create_sub_impl::<std_msgs::ByteMultiArray>(topic, Some(type_info))
                .with_qos(qos);
            let zsub = sub_builder.build().map_err(|e| e.into_pyerr())?;
            let wrapper = ZSubWrapper::new(zsub);
            Ok(PyZSubscriber::new(Box::new(wrapper), msg_type.to_string()))
...
fn create_client_for_type(
    ...
    match srv_type {
        "example_interfaces/srv/AddTwoInts" => {
            let client_builder = node.create_client_impl::<srv::AddTwoInts>(service, Some(type_info));
        ...
        _ => Err(pyo3::exceptions::PyTypeError::new_err(
            format!("Service type '{}' is registered but not implemented in client factory. \
...
fn create_server_for_type(
    ...
    match srv_type {
        "example_interfaces/srv/AddTwoInts" => {
            let server_builder = node.create_service_impl::<srv::AddTwoInts>(service, Some(type_info));
        ...
        _ => Err(pyo3::exceptions::PyTypeError::new_err(
            format!("Service type '{}' is registered but not implemented in server factory. \
                     Supported types: example_interfaces/srv/AddTwoInts",
                    srv_type)
        ))
    }
